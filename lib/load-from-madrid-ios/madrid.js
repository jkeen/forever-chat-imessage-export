...
function buildAttachmentRow(row, attachmentInfo) {
  return {
...
function mapMadridAttachment(id, row) {
  // console.log(row);
  if (row._attachment_info && row._attachment_info.length > 0) {
    ...
    _.each(attachmentGuids.match(/\w{8}-\w{4}-\w{4}-\w{4}-\w{12}/g), function(guid) {
      ...
      var attachmentInfo = madridAttachmentTableMap[guid];

      if (attachments && attachments.length > 0) {
        attachments.push(buildAttachmentRow(row, attachmentInfo));
        attachmentMap[id] = attachments;
      }
      else {
        attachmentMap[id] = [buildAttachmentRow(row, attachmentInfo)];
      }
    });
  }
}
...
function fetchResultsWithQuery(path, query) {
...
function fetchMadridAttachmentsTable(path) {
  var result = {};

  var promise = new RSVP.Promise(function(resolve, reject) {
    getConnection(path).then(function(db) {
      db.serialize(function() {
        db.each('SELECT * from madrid_attachment', function(error, row) {
          // get an overall map for relating data
...
module.exports = function(path, version, options) {
  // we need to relate this data outside of SQL since we can't do a join
  // between the tables due to the data being stuffed inside a blob field
  return fetchMadridAttachmentsTable(path).then(function(attachmentsTable) {
    madridAttachmentTableMap = attachmentsTable;
    var imessageQuery  = loadQuery("ios5_imessage.sql", options);
    return fetchResultsWithQuery(path, imessageQuery);
  });
}
