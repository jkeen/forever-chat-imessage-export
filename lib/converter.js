  ...
  buildPayload() {
    ...
    for (var i = 0; i < ordered.length; i++) {
      ...
      if (row) {
        ...
        message.me               = this.formatPhone(message.me);
        message.address          = this.formatPhone(message.address);

        message.attachments      = this.lookupAttachments(_id);
        message.participants     = this.lookupParticipants(row.message_group);
        message.participants     = _.uniq(_.map(message.participants, (p) => this.formatPhone(p)));
