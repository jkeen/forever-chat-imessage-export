...
function fetchMadridRecords(path, version, options) {
  return new RSVP.Promise((resolve, reject) => {
    return fetchMadridAttachmentsTable(path).then((attachmentsTable) => {
...
function fetchSMSRecords(path, version, options) {
...
module.exports = function fetchResults(path, version, options) {
  let madridRecords = [];
  let smsRecords    = [];
  return fetchMadridRecords(path, version, options).then(madridRecords => {
    return fetchSMSRecords(path, version, options).then(smsRecords => {
